...
- 配信元(GitHub Pages): https://4getkun.github.io/Curation_Jleague/ — 直接開くと上の公開URLへ自動転送されます
...
利用者向けの公開URLは `https://fourgetkun.com/jleague-news/` で、fourgetkun-hub の Worker(`src/pages-proxy/proxy.js`)が
`/jleague-news/*` へのリクエストを `https://4getkun.github.io/Curation_Jleague/*` から取得して返すリバースプロキシになっています。

- `astro.config.mjs` の `site` / `base` は公開側(`https://fourgetkun.com` / `/jleague-news`)に合わせています。
  そのため GitHub Pages 上のHTMLを github.io のURLで直接開くとCSS等が読めませんが、`Base.astro` 冒頭の
  インラインスクリプトが同じページの `fourgetkun.com/jleague-news/...` へ即座に転送します(旧URLの共有リンクも生きます)。
- `fourgetkun.com/Curation_Jleague/...` へのアクセスも hub 側で `/jleague-news/...` へ301転送します。
...
- 以下のセットアップ手順・ローカル確認のURL(`/Curation_Jleague/`)は移行前の記述です。ローカル開発は
  `npm run dev` → `http://localhost:4321/jleague-news/` で確認します。
...
- **今季の見通し(姉妹サイト連携)** — クラブページの右列に、姉妹サイト「[J1順位確率早見表](https://fourgetkun.com/j1-odds/)」の現在順位・優勝/ACL圏/降格確率・次の試合の予想を表示。ビルドのたびに `scripts/fetch-odds.mjs` が最新の計算結果を取得して `src/data/odds-j1.json` に保存する(取得に失敗しても前回のデータで公開を続ける)。クラブの対応は `src/data/teams.json` の `oddsId`。昇格・降格でクラブが入れ替わり、確率サイトのクラブと食い違うとActionsに警告が出る
...
Claude Designによるデザイン案(デザインハンドオフ一式)を導入し、姉妹サイト「Curation NPB」から独立した
専用デザインシステムに刷新済みです。クリーム地+ダークグリーン基調、見出しはNoto Serif JP、数字・英語表記は
Oswald、本文はNoto Sans JPを使用したエディトリアル/マガジン調で、角丸なし・罫線中心の誌面レイアウトを採用
しています。トップページにはクラブカラー付きの日中/夜間ヒーロー画像(`astro:assets`の`<Image>`で最適化・
`.dark`クラスに応じて自動切り替え)を配置し、ニュース一覧はOswald書体の連番付きリストで統一しています。

配色はすべて `src/styles/global.css` の `:root` / `.dark` に定義したCSS変数(`--page-bg` `--text` `--accent`
`--card-bg` `--rule-mid` など)にまとまっているため、この変数を書き換えるだけでライト/ダーク両方の配色を
一括調整できます。コンポーネント側は `var(--xxx)` を参照しているだけなので、`dark:` バリアントを個別に書き足す
必要はありません。各クラブのアクセントカラーは `src/data/teams.json` の `color`(HEX)で管理し、明度から自動で
文字色・リングの要否を判定するロジックが `src/lib/teams.ts` にあります(`LIGHT_TEAM_COLORS` / `needsContrastRing`)。
PWAアイコン・OGP画像(`public/icons/` `public/og-image.png`)は今回のデザイン刷新の対象外で、NPB版からの流用
プレースホルダーのままなので、公開前にJリーグ向けの画像に差し替えることをおすすめします。

さらにデザインを調整する場合は、`src/styles/global.css` のテーマ変数と `src/components/` 配下(特に `NewsRow.astro`
`Hero.astro` `TeamLinkRow.astro` `TeamChipLink.astro` `Header.astro`)を中心に編集してください。ニュース一覧
(`news/index.astro`)とクラブ別ページ(`team/[team].astro`)のページネーションは、サーバー側ページングではなく
クライアントサイドJSで`.news-page-item`をグループ化して表示切り替えするだけの実装のため、`PAGE_SIZE`定数の
値を変えるだけで1ページあたりの件数を調整できます。
